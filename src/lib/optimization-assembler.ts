...
import { buildTalentAxes } from './talent-axes';
 ...
 * sources (gear, gems, enchants, talents). The combinator and combination
...
export function assembleAxes(
  ...
  const talentAxes = selectedLoadoutNames ? buildTalentAxes(profile, selectedLoadoutNames) : [];
  return [...gearAxes, ...gemAxes, ...enchantAxes, ...talentAxes];
}
